...
export const SearchBarNav = (props: Props) => {
  ...
  const searchOutsideRef = useRef<HTMLElement>(null)

  useOnClickOutside(searchOutsideRef, () => {
    isOpen && toggleOpen()
  })
  ...
  useEffect(() => {
    const escapeKeyDownHandler = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && isOpen) {
        ...
        focusKeyhandler(event)
      }
    }

    const focusKeyhandler = (event: KeyboardEvent) => {
      if (event.key === '/' && (event?.target as any)?.nodeName !== 'INPUT') {
        event.preventDefault()
              ...
              {!Boolean(fetching) && Boolean(results?.length) && (
                <React.Fragment>
                  ...
                  {!routing && results?.filter(isValidPair).map((result) => (
                    <div style={{ zIndex: 99, alignItems: 'center', cursor: 'pointer' }} key={result.pairAddress} onClick={(e) => {
                      onPairClick(result)
                    }}>
                      <MenuItemStyled style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} >
